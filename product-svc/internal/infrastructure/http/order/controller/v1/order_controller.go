...
func NewOrderController(orderService usecase.OrderUseCase) *OrderController {
	return &OrderController{
		logger: config.ContextLogger.WithFields(log.Fields{
			"type": "controller:OrderController",
		}),
		orderService: orderService,
	}
}

func (h *OrderController) GetDetailedOrder(c *gin.Context) {
	...
	res, err := h.orderService.GetDetailedOrder(c.Request.Context(), userId, uint64(id))
	if err != nil {
		h.logger.WithError(err).Error("GetDetailedOrder")
